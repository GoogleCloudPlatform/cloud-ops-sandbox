...
from sre_recipe_utils import init_sre_recipe_web_ui
...
def setup_locust_environment(user_classes,
                             shape_class,
                             headless,
                             web_host,
                             web_port,
    ...
    env = Environment(
        host=FLAGS.host, user_classes=user_classes, shape_class=shape_class)
    ...
    if not headless and not FLAGS.worker:
...
def main(argv):
    ...
    env_sre_recipe = setup_locust_environment(
        user_classes=[],
        shape_class=None,
        headless=FLAGS.headless_sre_recipe,
    ...
    if env_sre_recipe.web_ui is not None:
        init_sre_recipe_web_ui(env_sre_recipe.web_ui)

        # Start the SRE Recipe web UI, if not in headless mode
        if not FLAGS.headless_sre_recipe:
