...
applyTerraform() {
  local state_prefix=${TERRAFORM_STATE_PREFIX:-""}
  ...
  pushd "./terraform"
  terraform init -reconfigure -backend-config "bucket=${bucket_name}" -backend-config "prefix=${state_prefix}" -lockfile=false #2> /dev/null
  ...
  if [[ -z "${skip_loadgen}" ]]; then
    ...
    terraform_command+=" --var=\"filepath_manifest=../kustomize/online-boutique/without-loadgenerator/\"" 
  fi
