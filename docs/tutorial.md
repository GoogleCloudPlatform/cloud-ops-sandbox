...
The Hipster Shop application consists of a number of microservices, written in a variety of languages, that talk to each other over gRPC. [To learn more](<addlink to guide>
...
Cloud Operations provides products for both developers and administrators; this section introduces a few of the products, for additional information refer to the [User Guide] (<link>).
...
Note: For application instrumentation your applications, the recommended solution is [**OpenCensus**](https://opencensus.io/), an open-source project that supports trace instrumentation in a variety of languages and that can export this data to Cloud.
...
In addition, Cloud Operations provides the several tools that are useful for [Site Reliability Engineering (SRE)](<add link>):
...
For additional information and examples like log export see User Guide(<link>)  
