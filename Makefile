...
.-PHONY: cluster deploy deploy-continuous logs destroy check-env check-service
...
cluster: check-env
	...
	./terraform/istio/install_istio.sh; \
	skaffold run -f ./skaffold.yaml --default-repo=gcr.io/$${PROJECT_ID}/sandbox -l skaffold.dev/run-id=${CLUSTER}-$${PROJECT_ID}-${ZONE}
