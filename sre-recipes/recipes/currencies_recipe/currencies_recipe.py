...
class CurrenciesRecipe(Recipe):
    ...
    def _deploy_state(state):
        ...
        if not loadgen_ip:
            ...
            print(f"No load generator IP found. Will not {operation} synthetic load")
            logging.error(f"No load generator IP found. Will not {operation} synthetic load")
        elif not state:
            # Stop generating load first, before restarting the frontend
            loadgen_command = "curl -X POST http://{loadgen_ip}:81/api/stop"
        ...
        if state:
            ...
            loadgen_command = """curl -X POST http://{loadgen_ip}:81/api/spawn/BaiscHomePageViewingUser \
                -F 'user_count=20' -F 'spawn_rate=5' -F 'stop_after=600'"""
            logging.info(f"Trying to start load generation: {loadgen_command}")
            resp, err = Recipe._run_command(loadgen_command)
            if resp is not None:
              resp = resp.decode("utf-8").strip()
            logging.info(f"Load Generator API Response: {resp}")
